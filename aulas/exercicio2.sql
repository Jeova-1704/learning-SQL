...
/* Considerando o que vs aprendeu sobre performance, resolva os exercicios abaixo */
-- Contar a quantidade de registros da minha tabela
...
-- primeiramtne eu separei a quantidade de pessoas no departamento filmes e roupas, para depois ver quantos tem em cada para poder fazer a query com performace: (vendo isso analisei que a maior quantidade era de roupas, sendo essa 53 e de filmes igual a 21)
SELECT departamento, COUNT(*) as quantidade_funcionarios FROM funcionarios WHERE departamento IN ('roupas', 'filmes') GROUP BY departamento;
--Agora vou fazer a query para trazer os funcionarios que trabalham no departamento de filmes ou no de rouaps
SELECT * FROM funcionarios WHERE departamento = 'roupas' OR departamento = 'filmes';


--2° o gestro de marketing pediu a lista das funcionarias que trabalhem no departamento de filmes ou no departamento lar. Ele nescessita enviar um email para colaboradores desses dois setores 
-- primeiramtne eu separei a quantidade de pessoas no departamento filmes e lar, para depois ver quantos tem em cada para poder fazer a query com performace: (LAR TEM MAIS FUNCIONARIOS DO QUE FILMES, 32 VS 9 )
SELECT departamento, COUNT(*) AS quantidade_funcionarios FROM funcionarios WHERE departamento in ('filmes', 'lar') and sexo = 'Feminino' GROUP by departamento;

-- Solução com a melhor performace;
SELECT * FROM funcionarios WHERE (sexo='Feminino') and (departamento = 'lar' or departamento = 'filmes');


--3° Traga os funcionarios do sexo masculino ou os funcinarios que trbalhem no setor jardin 
--Analisando a quantidade de cada tipo novamente para poder desenvolver uma query com maior performace
SELECT sexo, COUNT(*) AS quantidade_funcionarios FROM funcionarios WHERE sexo = 'Masculino';
SELECT sexo, COUNT(*) AS quantidade_funcionarios FROM funcionarios WHERE sexo = 'Feminino' and departamento = 'Jardim';
-- tem 484 pessoas do sexo masculino e 23 mulheres que trabalham no departamentoo do Jardin

-- Resposta da query:
SELECT * FROM funcionarios WHERE (sexo = 'Masculino') or (sexo = 'Feminino' and departamento = 'Jardim');
